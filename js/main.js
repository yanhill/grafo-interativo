...
function initSigma(config) {
	...
	if (config.sigma && config.sigma.drawingProperties) //(acho) SE JÁ HOUVER PRE PRORIEDADES DE DESENHO
		drawProps=config.sigma.drawingProperties;
	else // SE NÃO OUVIR PRE PROPRIEDADES DE DESENHO
		drawProps={ //
    ...
    if (config.sigma && config.sigma.graphProperties)	
    	...
    	graphProps={ // PROPRIEDADES DE GRÁFICO
        minNodeSize: 1,
        maxNodeSize: 7,
        minEdgeSize: 0.1,
        maxEdgeSize: 0.9
	...
	if (config.sigma && config.sigma.mouseProperties) //PRIPRIEDADES DE MAUSE
		mouseProps=config.sigma.mouseProperties;
	else
		mouseProps={ 
        minRatio: 0.25, // How far can we zoom out?
        maxRatio: 30, // How far can we zoom in?
    	};
    
    //CHAMANDO O SIGMA COMO UMA VARIAVEL
...
function setupGUI(config) {
    ...
    $GP.menuPrincipal = $("#mainTitle");
    
    $GP.selection = $("#selection");
    $GP.selection_atores = $GP.selection.find("#containerAtores");
    $GP.selection_atores_sum = $GP.selection_atores.find("#dadoNumerico");
    $GP.selection_retweets = $GP.selection.find("#containerRetweets");
    $GP.selection_retweets_sum = $GP.selection_retweets.find("#dadoNumerico");

    $GP.grupos = $(".sessao-grupos");
    $GP.grupos_gpContainer =  $GP.grupos.find(".container-do-grafico");

    $GP.porcentagem = $(".sessao-porcentagem");
    $GP.porcentagem_gpContainer =  $GP.porcentagem.find(".container-do-grafico");

    $GP.actors = $(".sessao-actors");
    $GP.actors_container = $GP.actors.find(".container-do-grafico")
...
function Cluster(a) {
    ...
    console.log($GP.grupos_gpContainer.find(".grupo"));
...
function showCluster(a) {
    // b adquire lista dos nós do cluster clicado
